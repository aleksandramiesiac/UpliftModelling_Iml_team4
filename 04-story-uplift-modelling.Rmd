...
There are two records per person, one for recurrence and one for death. Dataset contains 1858 observations and 16 features which are described in the table \@ref(tab:04-variables-description).
...
Typically, the survival data includes two variables: the observed survival time (`time` variable in our dataset) and a binary censoring status variable (`status` variable in our dataset). The `status` variable indicates whether the event has been observed (typically denoted by `status = 1`). However, if the event has not been observed (`status = 0`) then the true survival time has ben censored, i.e. it is only known that the true survival time is at least equal to `time`.
<!-- TODO: citation for: https://home.ipipan.waw.pl/s.jaroszewicz/pdf/HI_KDD14.pdf -->
...
Distribution of the variable `rx` is as shown in the table \@ref(tab:04-rx-distribution). This feature has been categorised.
...
The final model is `XGBTRegressor` with parameters summarised in the table \@ref(tab:04-model-params).
...
The two observations which we have selected can be found in the table \@ref(tab:04-selected-obs).
...
#### Break Down
```{r, echo=FALSE,out.width="45%", out.height="40%",fig.cap="Break Down",fig.show='hold',fig.align='center'}
...
#### LIME
```{r, echo=FALSE,out.width="49%", out.height="20%",fig.cap="Lime",fig.show='hold',fig.align='center'}
